---
name: notion-task-automation
description: Notion任务自动化管理技能 - 完整版。自动检查任务状态、产品经理分析、自动开发、测试报告生成、Bug修复跟踪、产品验收、进度追踪、可视化看板。
triggers:
  - cron: "*/30 * * * *"
  - message: "检查Notion任务"
  - message: "执行任务开发"
  - message: "生成任务看板"
version: 2.5
author: AI Assistant
date: 2026-03-05
---

# Notion 任务自动化管理 SKILL v2.5

## 🎯 功能概览

```
┌─────────────────────────────────────────────────────────┐
│            Notion Task Automation Skill v2.5            │
├─────────────────────────────────────────────────────────┤
│  阶段一：开发                                            │
│  ├── 1. check       - 检查任务状态，生成报告             │
│  ├── 2. execute     - 自动执行确认迭代的任务             │
│  └── 3. dashboard   - 生成可视化看板                    │
│                                                         │
│  阶段二：测试（新增）                                     │
│  ├── 4. test-report    - 自动生成测试报告               │
│  └── 5. bug-track      - Bug修复任务跟踪                │
│                                                         │
│  阶段三：验收（新增）                                     │
│  └── 6. acceptance  - 产品验收通知                     │
│                                                         │
│  系统功能                                                │
│  └── 7. status      - 查看系统状态                      │
└─────────────────────────────────────────────────────────┘
```

## 🚀 快速开始

### 1. 配置环境变量

创建 `.env` 文件：
```bash
NOTION_TOKEN=your_notion_integration_token
NOTION_DATABASE_ID=your_database_id
```

### 2. 运行命令

```bash
# 完整自动化流程（检查 + 执行）
./automation.sh full

# 只检查任务状态
./automation.sh check

# 只执行确认迭代的任务
./automation.sh execute

# 生成可视化看板
./automation.sh dashboard

# 查看系统状态
./automation.sh status
```

## 📊 工作流程

```
Notion任务状态流转（三阶段）：

阶段一：开发
┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐
│  未开始   │───▶│ 待确认   │───▶│ 确认迭代 │───▶│  进行中   │
│  (新任务) │    │ (分析中) │    │ (准备开发)│    │ (开发中) │
└──────────┘    └──────────┘    └────┬─────┘    └────┬─────┘
                                      │               │
                                      │ 自动触发       │
                                      ▼               ▼
                              ┌────────────────┐  ┌──────────┐
                              │ 产品经理分析    │  │ 自动开发  │
                              │ 生成V1.1 PRD   │  │ 子代理   │
                              └────────────────┘  └────┬─────┘
                                                       │
                                                       ▼
阶段二：测试                                    ┌──────────┐
┌──────────┐    ┌──────────┐    ┌──────────┐   │ 测试中   │
│  Bug修复  │◀───│ 修复中   │◀───│ 测试失败 │◀──┤ (待测试) │
│  重新测试 │    │ (修Bug)  │    │ 生成报告 │   └────┬─────┘
└────┬─────┘    └──────────┘    └──────────┘        │
     │                                               │ 测试通过
     └───────────────────────────────────────────────┘
                                                       ▼
阶段三：验收                                    ┌──────────┐
                                               │ 待验收   │
                                               │ (待验收) │
                                               └────┬─────┘
                                                    │
                                                    ▼
                                               ┌──────────┐
                                               │  已完成   │
                                               │ (已部署) │
                                               └──────────┘
```

## 🔄 自动触发机制

### 定时触发（默认30分钟）
- **每30分钟**自动执行 `full` 命令
- 检查是否有状态变更的任务
- 自动触发相应流程

### 状态变更立即响应 ⭐
当任务状态在Notion中被修改时，系统会**立即检测**并执行相应操作，无需等待30分钟：
- 新建任务（未开始）→ 立即开始产品经理分析
- 改为"确认迭代" → 立即开始开发
- 测试完成 → 立即通知验收

### 手动触发
发送消息到 Feishu：
- `检查Notion任务` → 立即执行 check
- `执行任务开发` → 立即执行 execute
- `生成任务看板` → 立即执行 dashboard

## 📁 文件结构

```
skills/notion-task-automation/
├── SKILL.md              # 技能文档（本文档）
├── automation.sh         # 核心自动化脚本
├── state.json            # 状态存储（自动创建）
└── dashboard.html        # 可视化看板（自动生成）
```

## 📝 日志和调试

### 日志文件
```
/tmp/notion-skill.log     # 详细执行日志
```

查看实时日志：
```bash
tail -f /tmp/notion-skill.log
```

### 状态文件
```
skills/notion-task-automation/state.json
```

存储：
- 最后检查时间
- 开发任务状态
- 开发目录映射

## 🎨 可视化看板

生成后可以在浏览器中打开：
```bash
# 生成看板
./automation.sh dashboard

# 打开看板
open skills/notion-task-automation/dashboard.html
```

看板显示：
- 任务统计卡片
- 任务列表及状态
- 最后更新时间

## 🔧 高级配置

### 自定义检查频率

编辑 crontab：
```bash
crontab -e
```

修改为所需频率：
```bash
# 每5分钟（默认）
*/5 * * * * /path/to/automation.sh full

# 每10分钟
*/10 * * * * /path/to/automation.sh full

# 每小时
0 * * * * /path/to/automation.sh full
```

### 扩展功能

在 `automation.sh` 中添加自定义步骤：
```bash
# 在 execute_confirmed_tasks 函数中添加
custom_step() {
    log "执行自定义步骤..."
    # 你的代码
}
```

## 🐛 故障排除

### 问题1：缺少环境变量
```
错误: 缺少 NOTION_TOKEN 或 NOTION_DATABASE_ID
```
**解决：** 检查 `.env` 文件是否正确配置

### 问题2：缺少 jq 命令
```
错误: 缺少 jq 命令
```
**解决：** `sudo apt-get install jq`

### 问题3：Notion API 错误
```
错误: API 请求失败
```
**解决：** 
1. 检查 NOTION_TOKEN 是否有效
2. 确认数据库已分享给 Integration
3. 查看 `/tmp/notion-skill.log` 获取详细错误

## 📝 更新记录

### v2.5 (2026-03-05) - 测试与验收阶段
- ✅ **测试报告自动生成** - 测试中状态自动生成报告
- ✅ **Bug修复任务跟踪** - 测试失败自动创建修复任务
- ✅ **产品验收通知** - 待验收状态自动通知产品经理
- ✅ **新增状态支持** - 测试中、待验收、修复中
- ✅ **三阶段工作流** - 完整覆盖开发→测试→验收流程
- ✅ **错误处理增强** - 网络错误自动重试
- ✅ **日志分级系统** - DEBUG/INFO/WARN/ERROR 四级日志

### v2.0 (2026-03-04)
- ✅ 完整的任务状态检查
- ✅ 自动执行确认迭代的任务
- ✅ 可视化看板生成
- ✅ 详细日志记录
- ✅ 状态持久化
- ✅ 完善的错误处理
- ✅ 帮助文档

### v1.0 (2026-03-04)
- 🎉 初始版本
- ✅ 基础任务检查
- ✅ 简单通知功能

## 💡 使用建议

1. **定期检查日志**：`tail -f /tmp/notion-skill.log`
2. **查看看板**：定期生成 dashboard 查看整体进度
3. **及时处理待确认任务**：待确认任务需要用户决策，不要堆积
4. **监控开发中的任务**：确保子代理正常完成开发

## 🔮 未来规划

- [x] **测试报告自动生成** ✅ v2.5
- [x] **Bug修复任务跟踪** ✅ v2.5
- [x] **产品验收通知** ✅ v2.5
- [x] **错误处理增强** ✅ v2.5
- [x] **日志分级系统** ✅ v2.5
- [ ] Webhook 触发支持
- [ ] 多数据库支持
- [ ] 任务优先级自动排序
- [ ] 开发时间预估
- [ ] 部署状态追踪
- [ ] Web管理界面
- [ ] 多渠道通知支持

---

**作者：** AI Assistant  
**版本：** 2.5  
**日期：** 2026-03-05
